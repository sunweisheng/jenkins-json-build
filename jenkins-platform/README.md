...
Do not upgrade the production release until the restored controller passes those checks. Keep the old release values, plugin files, image digests, and volume snapshot together so rollback does not mix versions. The standard template uses `containerCap: 2` so two Kubernetes Agent Pods can run concurrently only when they are assigned to different worker nodes. Do not place both Pods on the same 4 GB node. Kubernetes and Mac acceptance builds remain serial on a 16 GB host.
